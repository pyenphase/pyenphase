...
if envoy.actual_phase_count > 1 and data.system_production_phases:
    ...
    for phase in data.system_production_phases:
        ...
        print(
            f"{phase} TodaysEnergy: {data.system_production_phases[phase].watt_hours_today}"
        )
        print(
            f"{phase} LifetimeEnergy {data.system_production_phases[phase].watt_hours_lifetime}"
        )
        print(
            f"{phase} Last7DaysEnergy {data.system_production_phases[phase].watt_hours_last_7_days}"
        )
